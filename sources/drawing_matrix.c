...
/*   Updated: 2019/12/16 16:51:46 by ymanilow         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				ft_draw_matrix_img_iso(t_pointers *p)
{
	...
	while (++j < p->mtrx.ln_y)
	{
		while (++i < p->mtrx.ln_x)
		{
			p->dot_s.x = i * p->base.size;
			p->dot_s.y = j * p->base.size;
			ft_iso(&p->dot_s.x, &p->dot_s.y, p->mtrx.array[j][i], p);
			p->dot_s.x += p->base.start_x;
			p->dot_s.y += p->base.start_y;
			p->dot_e.y = j * p->base.size;
			if (i != p->mtrx.ln_x - 1)
			{
				p->dot_e.x = (i + 1) * p->base.size;
				ft_iso(&p->dot_e.x, &p->dot_e.y, p->mtrx.array[j][i + 1], p);
				p->dot_e.x += p->base.start_x;
				p->dot_e.y += p->base.start_y;
			...
			p->dot_e.x = i * p->base.size;
			if (j != p->mtrx.ln_y - 1)
			{
				p->dot_e.y = (j + 1) * p->base.size;
